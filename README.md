...
- 전 영역 스케일 균일도: **±0.12 mm** (20 mm 셀 기준, 6개 보드 교차 측정)

> **앵커 보드 주의**: 호모그래피 기준 보드는 반드시 *깨끗하게 검출되는* 보드를 써야 합니다.
> 노이즈가 있는 중앙 보드(044428)에 앵커링하면 전 영역에 상하 스케일 기울기가 생겨 균일도가
> 0.48 mm로 나빠집니다. 그래서 축정렬된 깨끗한 보드(075337)를 기준으로 쓰고, 출력 프레임은
> 원본 이미지 방향에 맞춰 재정렬합니다.
